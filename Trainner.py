...
class motorControl:
    ...
    def __init__(self):
        self.control = RpiMotorLib.A4988Nema(self.dir_pin, self.stp_pin, (21,21,21), "DRV8825")
        GPIO.setup(en_pin, GPIO.OUT)
        self.pos = 0

    def move(self, delta, cw):
        """
        delta being the degrees of movement
        """
        GPIO.output(en_pin, GPIO.LOW)
        self.control.motor_go(cw, "Full", int(200*15 * delta/360), 0.01, False, 0.05)
...
class FitnessMachineControlPointCharactaristic(Characteristic, CommonData):
    ...
    def SIMparams(self, CommonData):
        ...
        if (self.temp_grade < 2000) and (self.temp_grade > -2000):
...
class FitnessMachineStatusCharactaristic(Characteristic):
    """
